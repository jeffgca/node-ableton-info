...
export class AbletonPrefs {
	...
	/**
	 * Extract VST3 custom path from bytes - works for both macOS and Windows
	 *
	 ...
	 */
	extractVst3CustomPathUnified(bytes) {
	...
	extractVst3CustomPath(bytes) {
		...
		if (this.#_os === 'win32') {
			...
			return this._extractWin32Vst3CustomPath(bytes)
		} else {
			// Non-Windows (e.g., macOS) extraction logic
			return this._extractMacOSVst3CustomPath(bytes)
		}
	}
	 ...
	 * warning: this will fail on windows
	 */
	_extractMacOSVst3CustomPath(bytes) {
		let textSegment = this.bytesToStringArr(bytes)
		let filtered = textSegment
			.filter((char) => {
				if (char !== '\\x00') return true
	...
	_extractWin32Vst3CustomPath(bytes) {
		let textSegment = this.bytesToStringArr(bytes)

		console.log('textsegment', bytes, textSegment)
		...
		let regex = new RegExp('([A-Z]\:[\/\w]+)\\x15', 'g')

		// console.log('filtered', filtered)

		if (regex.test(filtered)) {
			let path = regex.match(filtered)[1]
